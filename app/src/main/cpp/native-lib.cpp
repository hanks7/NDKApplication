...
Java_com_easyway_ndkapplication_jni_JNIAccessField_accessFile(JNIEnv *env, jobject instance,
    ...
    jfieldID nameFiledId = env->GetFieldID(beanClass, "name", "Ljava/lang/String;");//得到name字段的fieldID
    jstring joldStrName = static_cast<jstring>(env->GetObjectField(bean, nameFiledId));//1.先得到jfieldID为nameFiledId的object类型 2.再强转为jstring类型
